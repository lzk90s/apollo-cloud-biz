...
def get_stock_industry():
    ...
    db.StockIndustry.insert_many(data_list).on_conflict_replace().execute()
...
def get_latest_n_day_stock_k_data(stock_code, n_days):
    ...
    db.StockKData.insert_many(data_list).on_conflict_replace().execute()
...
def test():
    url = "http://quotes.money.163.com/0600570.html#2u01"
    resp = requests.get(url)
    soup = BeautifulSoup(resp.text)
    hq_details = soup.find("tr", attrs={'class': 'stock_bref'})
    if not hq_details:
        return None
    print(hq_details)
