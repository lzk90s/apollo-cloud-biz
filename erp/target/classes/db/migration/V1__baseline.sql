...
create TABLE `t_community`  (
  `id` bigint(10) NOT NULL AUTO_INCREMENT COMMENT '商品id',
  `code` varchar(64) NOT NULL COMMENT 'id',
  `name` varchar(64) NOT NULL COMMENT '名字',
  `area` varchar(64) NOT NULL DEFAULT '' COMMENT '所属区域',
  `address` varchar(128) NOT NULL DEFAULT '' COMMENT '地址',
  `delivery_year` int(4) NOT NULL DEFAULT 0 COMMENT '交付年',
  `ownership_type` varchar(32) NOT NULL DEFAULT '' COMMENT '权属类别',
  `price` int(10) NOT NULL DEFAULT 0 COMMENT '价格',
  `total_household` int(4) NOT NULL DEFAULT 0 COMMENT '总户数',
  `deleted` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否已删除',
  `add_time` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '添加时间',
  `update_time` timestamp NOT NULL DEFAULT current_timestamp() ON update current_timestamp() COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
