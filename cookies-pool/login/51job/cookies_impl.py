...
class Job51MailCookiesGenerator(CookiesGenerator):
    website = '51job'

    async def new_cookies(self, username, password):
        return await Job51MailCookiesProvider(username, password, self.browser).main()


class Job51MailCookiesProvider(CookiesProvider):
    login_home_url = "https://accounts.Job51.com/signin/v2/identifier?continue=https%3A%2F%2Fmail.Job51.com%2Fmail%2F&service=mail&sacu=1&rip=1&flowName=GlifWebSignIn&flowEntry=ServiceLogin"
