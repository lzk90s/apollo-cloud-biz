...
class BaseSpider:
    ...
    def parse(self, response):
        ...
        for task in CRAWLER_TASKS:
            if self.exists(url, task['name']):
                ...
                if parse_rule:
                    items = func(response, task['name'], **parse_rule)
                else:
                    items = func(response, task['name'])

        for item in items:
            yield item

    def parse_common(self, response, task_name, pre_extract_method='xpath',
    ...
    def parse_json(self, response, task_name, detail_rule, ip_key='ip', port_key='port'):
        """
    ...
    def parse_raw_text(self, response, task_name, pre_extract=None, delimiter='\r\n', redundancy=None, protocols=None):
        """
        Raw response parser
        :param response: scrapy response
        :param task_name: task name
